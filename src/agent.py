...
class LegalAgent:
    ...
    def _run_retrieve(question: str, top_k: int = TOP_K, agent=None) -> str:
        ...
        references = [
            agent._format_reference(index, doc)
            for index, doc in enumerate(docs, start=1)
        ]
        references = agent._merge_references_by_source_case(references)
    ...
    def retrieve(self, question, top_k=None):
        ...
        return self._merge_references_by_source_case(references)
    ...
    def _deterministic_chat(self, question, analysis, history=None, memory=None):
        ...
        references = self._merge_references_by_source_case(references)
        if not references:
            return {
                "question": question,
                "intent": analysis.intent,
    ...
    @staticmethod
    def _merge_references_by_source_case(references):
        grouped = {}
        for ref in references:
            source_case_id = ref.get("source_case_id") or f"chunk-{ref.get('id')}"
            if source_case_id not in grouped:
                grouped[source_case_id] = {
                    **ref,
            ...
            current = grouped[source_case_id]
            content = ref.get("content", "")
            if content and content not in current.get("content", ""):
                current["content"] = f"{current.get('content', '')}\n\n{content}".strip()
            current["chunk_count"] = current.get("chunk_count", 1) + 1
            if not current.get("accusations") and ref.get("accusations"):
                current["accusations"] = ref.get("accusations", "")
            if not current.get("articles") and ref.get("articles"):
                current["articles"] = ref.get("articles", "")
            if not current.get("punishment") and ref.get("punishment"):
                current["punishment"] = ref.get("punishment", 0)
